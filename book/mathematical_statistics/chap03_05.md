...
Moreover, since the inner of $$\{\theta \in \Theta\}$$ is not empty, by theorem 3.18, $T^{*}$ is complete with respect to $$\{P_{(b, \theta^{*})}\}_{\theta^{*} \in \Theta^{*}}$$.
...
With out loss of generality, function $g: \mathrm{Range}(T) \rightarrow [0, 1]$.

for all test $\phi$, $$\mathrm{E}_{\theta}[\phi \mid T]$$ is a test.
Indeed, by theorem 3.14, $(T_{1}, \ldots, T_{m})$ is sufficinet with respect to $$\{P_{\theta}\}_{\theta \in \Theta}$$ so that there exists stati
...
Let $$(b, b^{*}), (\theta_{1}, \theta^{*}) \in \Theta$$ be fixed.
...
For simplicity, here we write $\Theta_{i}^{\prime}$ as $\Theta_{\prime}$.
...
All unbiased test satisfies $$\eqref{chap03_03_36}$$ and $$\eqref{chap03_03_37}$$.
