...
By theorem 3.14, $$T^{*}$$ is sufficient to $$\{P_{(b, \theta^{*})}\}_{\theta^{*} \in \Theta^{*}}$$.
...
Let $$(b, b^{*}) \in \Theta_{0}, (\theta_{1}, \theta^{*}) \in \Theta_{1}$$ be fixed.
...
\begin{eqnarray}
    ...
    & := &
        \left\{
            ...
            \int_{\mathbb{R}^{m_{1}}}
                \exp
                \left(
                    \langle \bar{\theta}_{1:m_{1}}, t_{1:m_{1}} \rangle
                \right)
            \ \mu_{\bar{\theta}^{*}}(d t_{1:m_{1}})
            =
            0
            \text{ or }
            \infty
    ...
    \nonumber
    \\
    P_{\bar{\theta}_{1:m}}^{T^{*}}(N_{\bar{\theta}})
    & = &
        0
    \nonumber
\end{eqnarray}
...
By taking $\mu := P_{\vartheta_{1}}(t^{*}, \cdot)$, $g(t_{1}) := e^{(\theta_{1} - b)t_{1}} K_{\vartheta_{0}}/K_{\vartheta_{1}}$, $f_{1}(t_{1}) = 1$, $f_{2}(t_{1}) = t_{1}$, $c_{1} := \alpha$ and $c_{2} := \alpha$, $\Phi_{(c_{1}, c_{2})}$ in generalized Neyman-Peason's lemma equals to $$\eqref{chap03_theorem_03_33_set_of_functions}$$.
...
$\mathcal{H}_{\alpha}$ contains all unbiased test at level $\alpha$.
Now we solve a maximization problem;

$$
    \bar{\phi} \in \mathcal{H}_{\alpha},
    \
    \text{ s.t. }
    \
    \forall \phi \in \mathcal{H}_{\alpha},
    \
    \forall \theta \in \Theta_{1},
    \
    \beta_{\phi}(\theta)
...
* $\mu_{0} \in \mathbb{R}$,
    * given
* $\sigma \in \mathbb{R}_{\ge 0}$,
    * given
* $$\Theta := \{\mu \in \mathbb{R}\}$$,
* $$\Theta_{0} := \{\mu_{0}\}$$,
* $$\Theta_{1} := \{\mu \in \Theta \mid \mu \neq \mu_{0}\}$$,
...
\begin{eqnarray}
    ...
    & = &
        ...
        \left(
            ...
            \theta_{2}
            \sum_{j=1}^{n}
                (x_{j} - \mu_{0} - (\frac{-\theta_{1}}{2\theta_{2}}))^{2}
        \right)
    \nonumber
    \\
    & = &
...
Now we consider the equivalent test.
For simplicity, here we write $\Theta_{i}^{\prime}$ as $\Theta_{i}$.
By theorem 3.33 and continuity of distribution, there exists unbiases UMP test $\phi_{0}$ at level $\alpha$ such that
...
$$
    ...
    \begin{cases}
        1
        &
        (T_{1}(x) < u_{1}(T_{2}(x)) \vee T_{1}(x) \ge u_{2}(T_{2}(x)))
        \\
        0
        &
        (u_{1}(T_{2}(x)) < T_{1}(x) < u_{2}(T_{2}(x)))
    \end{cases}
$$

where $u_{i}: \mathbb{R} \rightarrow \mathbb{R} \ (i = 1, 2)$ are measurable functions.
All unbiased test satisfies $$\eqref{chap03_03_36}$$ and $$\eqref{chap03_03_37}$$ so that $u_{i}$ should satisfy that
...
\begin{eqnarray}
    ...
    & \Leftrightarrow &
        ...
        -
        \int_{\mathbb{R}}
            1_{t_{1} \le u_{1}(t_{2})}
            +
            1_{t_{1} \ge u_{2}(t_{2})}
        p(d t_{1} \mid t_{2})
        =
        1
        -
