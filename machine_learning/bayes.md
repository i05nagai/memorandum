...
機械学習での目的は、観測された$N$個のデータの組$(x_{1}, \ldots, x_{N})$（つまり、$X_{1}(\omega), \ldots, X_{N}(\omega)$）から、$X$の分布を何らかの意味で決定するparameter $\theta := \Theta(\omega)$を求めるのが基本的な問題である。
...
$$
\begin{eqnarray*}
    \theta^{*} := \arg\max_{\theta} p_{\Theta | (X_{1}, \ldots, X_{N})}(\theta | (x_{1}, \ldots, x_{N}))
        & = &
            \left(
